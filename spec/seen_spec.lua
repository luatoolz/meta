describe("seen", function()
  ...
  it("test", function()
    ...
    assert.same({'any1','other1'}, iter.map(seen))

    assert.equal(seen, seen + 'ok1')
    assert.equal(seen, seen + 'ok1')
    assert.same({'any1','other1', 'ok1'}, iter.map(seen))
  ...
  it("test with empty table", function()
    ...
    assert.equal(4, #o)
    assert.same({'any', 'other2', 'ok', 'okokok'}, iter.map(iter(seen)))
    assert.same({'any', 'other2', 'ok', 'okokok'}, iter.map(seen))
    assert.same({'any', 'other2', 'ok', 'okokok'}, iter.map(iter(o)))
  ...
  it("test with non-empty table", function()
    ...
    assert.equal(7, #o)
    assert.same({'x', 'y', 'z', 'any', 'other3', 'ok', 'okokok'}, iter.map(iter(seen)))
    assert.same({'x', 'y', 'z', 'any', 'other3', 'ok', 'okokok'}, iter.map(seen))
    assert.same({'x', 'y', 'z', 'any', 'other3', 'ok', 'okokok'}, iter.map(iter(o)))
