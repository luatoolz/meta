describe("table", function()
  ...
  it("insert()", function()
    local o = table {"x", "y"}
    assert.is_nil(o:insert(nil))
    assert.is_nil(o:insert("q"))
  ...
  it("map", function()
    ...
    assert.same(table(), table.map(table({}), tostring))
    local b = table({"x", "y", "z"})
    assert.same({"x", "y", "z"}, table.map(b))
    assert.same(b, table.map(b))
    assert.same(b, table.map(b, tostring))
    assert.same(b, table.map(b, 'tostring'))
    assert.same(b, table.map(table.ivalues(b)))
    assert.same(b, table.map(table.ivalues(b), tostring))
    assert.same(b, table.map(table.ivalues(b), 'tostring'))
    assert.same({"X", "Y", "Z"}, table.map(table.ivalues(b), string.upper))
  end)
  it("filter", function()
    local not_nil = function(x) return x ~= nil end
    local withnil = table {"x", nil, "y", nil, "z"}
--    assert.equal(3, #withnil)
...
--    assert.same(table {"x"}, r)
  ...
  describe("__eq", function()
    it("table", function()
      assert.is_true(table {"x", "y", "a", "b", "c"} == (table {"x", "y"} .. table {"a", "b"} .. table {"c"}), '1')
...
--      assert.same(table({"x", "y", a = {}}), (table{"x", "y"} .. table {a = {}}), '3')
    ...
    it("cloned", function()
      ...
      local yes = true
      assert.is_true(table.__eq(x, yes))
      assert.is_true(table.__eq(x, true))
