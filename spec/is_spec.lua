describe("is", function()
  ...
  it("is.__unm", function()
    assert.is_table(is.similar)
    assert.is_function(-(is.similar))
  end)
  it("typed", function()
    assert.equal('meta/loader', is.typed(meta.loader))

    assert.typed(meta.loader)
    ...
    assert.typed(meta.module)
    assert.not_instance(meta.module)
    assert.factory(meta.module)
    assert.equal('meta/module', is.typed(meta.module))

    local mm = meta.module('meta')
    assert.typed(mm)
    assert.instance(mm)
    assert.not_factory(mm)
    assert.equal('meta/module', is.typed(mm))
  end)
end)
