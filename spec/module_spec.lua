describe('module', function()
  ...
  it("testdata.loader.noinit", function()
    ...
    assert.is_table(m)
    assert.equal('loader/noinit', m.id)
    assert.is_nil(m.file)
    assert.is_true(m.isdir)
  end)
  it("module.noneexistent", function()
    assert.equal('noneexistent', module('testdata/noneexistent').id)
    assert.equal('noneexistent', module(module('testdata/noneexistent')).id)
    assert.is_nil(module('noneexistent').ok)
  end)
  it("key", function()
    assert.equal('meta', module('meta').name)
    assert.equal('init1/file', module('testdata.init1.file').id)
    assert.equal('init1/file', module('testdata/init1/file').id)
  end)
--[[
  it("has", function()
...
--    local a = module('init2')
--    assert.equal('',table(a.isroot, a))
...
--  it(".files", function()
--    assert.equal('', module('testdata').modz)
--    assert.equal('', module('meta').files)
--  end)
...
--    assert.is_true(mod.recursive.torecursive)
    ...
    assert.is_table(mod)
    assert.same({a={ok='ok'}, b={ok='ok'}, c={ok='ok'}, d={ok='ok'}}, mod.loader .. true)
...
--[[
  it("recursive preload", function()
    local webapi2 = module("testdata.webapi").set.recursive(true).preload(true)
    local webapi = loader("testdata.webapi")
    assert.equal(true, module("testdata.webapi").preload)
    assert.equal(true, module(webapi).preload)
    assert.equal(true, module(webapi2).preload)
    assert.equal(true, module("testdata.webapi").recursive)
    assert.equal(true, module(webapi).recursive)
    assert.equal(true, module(webapi2).recursive)
...
--    assert.ends('assert.lua', mod.luafile)
--    assert.truthy(mod.d)
--    assert.is_true(mod.d.isdir)
--    assert.equal(16, #mod.d.mods)
    assert.is_function(mod.handler)
  end)
end)