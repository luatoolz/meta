...
--]]
  it("maxi", function()
    ...
    assert.equal(3, #table {"a", nil, "x"})
    --    assert.equal(3, #(table {"a", nil, "x"}))
    assert.equal(3, #{"a", nil, "x", a=1})
    assert.equal(3, #{b=2, "a", nil, "x", a=1})
  end)
  it("empty", function()
...
--      assert.values({'failed.lua', 'init.lua'}, module.files)
--      assert.values({'failed.lua', 'init.lua'}, table.map(module.files))
...
--[[  it("filter", function()
  ...
  it("find", function()
    ...
    assert.truthy(table {"x", "y", "z"}:find("x"))
    assert.is_nil(table {"x", "y", "z"}:find("q"))
  ...
  it("flatten", function()
    ...
    assert.same({}, table({}):flattened())
    assert.same({}, table():flattened())
    assert.same({"x"}, table({"x"}):flattened())
    assert.same({"x", "y"}, table {"x", "y"}:flattened())
    assert.same({"x", "a", "b", "y"}, table {"x", {"a", "b"}, "y"}:flattened())
    assert.same({"x", "y", "a", "b", "q", "w", "e"}, table {"x", "y", {"a", "b", {"q", "w", "e"}}}:flattened())
  end)
--[[
  it("size", function()
...
--]]
  ...
  it("__concat / multi concat", function()
    ...
    assert.same(table {"x", "y", "a", "b"}, table {"x", "y"} .. table.iter(table {"a", "b"}))
  end)
--[[
