...
function tuple.concatter(...) local t = tuple(...); return function(self) return self..t() end end
function tuple.adder(...)     local t = tuple(...); return function(self) return self+t() end end
function tuple.caller(...)    local t = tuple(...); return function(self) return self(t()) end end
function tuple.getter(...)    local t = tuple(...); return function(self) return self[t()] end end
function tuple.setter(...)    local t = tuple(...); return function(self) self[t[1]] = t[2] end end
function tuple.haver(...)     local t = tuple(...); return function(self) return self[t()] and self end end
...
__concat=function(self, it)
  ...
  local rv = tuple()
  if not rawequal(tuple, self) then for _,v in ipairs(self) do _=rv+v end end
  if type(it)=='function' then for v in it do _=rv+v end end
  if type(it)=='table' and ((not getmetatable(it)) or getmetatable(it).__name=='tuple') then
    for _,v in ipairs(it) do _=rv+v end
  else if it~=nil then _=rv+it end end
  return rv end end,
