...
local index, preserve, argz, maxi = mmt.i, pkg.preserve, pkg.argz, pkg.maxi
...
function table:update(...) if is.table(self) then
  for it in iter.tuple(...) do if type(it)=='table' then
    if table.indexed(it) then for v in iter(it) do table.append(self, v) end end
    for k,v in pairs(it) do if type(k)~='number' then self[k]=v end end
	end end; return self end end

-- searchers/selectors
function table.find(...) local swap=fn.swap; return swap(iter.find(...)) or nil end
...
function table:autopairs()
  ...
  if indexed then nexter=inext end
  return nexter, self
  end
